...
def get_ordinate_matrix(name):
    if type(name) == int:
        ...
        data = open(name, 'r').readlines()
        try:
            data = np.loadtxt(data, skiprows=9)[:, 2:]
        except:
            data = np.loadtxt(data, skiprows=15)[:, 2:]
    return data
...
def get_box(name):
    data = open(name, 'r').readlines()
    try:
        box = [data[3].split()[0], data[3].split()[1], data[4].split()[0], data[4].split()[1], data[5].split()[0], data[5].split()[1]]
    except:
        box = [data[5].split()[0], data[5].split()[1], data[6].split()[0], data[6].split()[1], data[7].split()[0], data[7].split()[1]]
